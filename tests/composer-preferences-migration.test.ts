...
it.each([false,true])('upgrades preference memory without rewriting chat history (saved choice: %s)',selected=>{
 ...
 db.exec(old);db.pragma('user_version=31'); const memory=memoryJson({character_id:'shared',revision:0,database_records:[]});db.prepare('INSERT INTO shared_memory VALUES(1,?,?)').run(memory,memoryHash(memory));db.transaction(()=>new StarterStore(db).initialize())();
 if(selected){
 ...
 expect(db.prepare('SELECT * FROM sessions').all()).toMatchObject(before);
