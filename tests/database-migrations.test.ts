...
it('migrates first public schema to current with a consistent recovery snapshot and exact fresh schema parity', () => {
  ...
  expect(db.pragma('user_version', { simple: true })).toBe(23); validateSchema(db, current);
...
it('rolls back a mid-migration failure and restarts without replacing the original backup', () => {
  ...
  expect(restarted.pragma('user_version', { simple: true })).toBe(23);
...
it('upgrades through real Store startup and retires unrequested grammar blockers without replay', () => {
  ...
  try {
    ...
    expect(store.session('second-old').analysis_state).toBe('none');
  ...
  expect(after.pragma('user_version',{simple:true})).toBe(23);
...
it.each([13,14,15,16,17,18])('admits index grammar from schema %i without rewriting old grammar state', (version) => {
  const { db, dir }=fixture(version===13?old:source18,version);
  ...
  migrateDatabase(db,dir);expect(db.pragma('user_version',{simple:true})).toBe(23);validateSchema(db,current);
...
it('upgrades schema 19 report source linkage while preserving immutable report bytes and rollback recovery',()=>{
  const {db,dir}=fixture(source19,19);db.transaction(()=>installCatalog19(db))();
  const snapshot='{"historical":"exact snapshot"}',request='{"historical":"exact request"}';
  db.prepare('INSERT INTO pattern_reports(id,created_at,fingerprint,snapshot) VALUES(?,?,?,?)').run('report','2026-09-01','hash',snapshot);
  db.prepare('INSERT INTO pattern_report_sources VALUES(?,?,?,?,?)').run('report','session','analysis','source-hash',0);
  db.prepare("INSERT INTO pattern_report_attempts(id,report_id,request,request_hash,status,created_at) VALUES(?,?,?,?,'queued',?)").run('attempt','report',request,'request-hash','2026-09-01');
  db.prepare("UPDATE pattern_report_attempts SET status='dispatched',dispatched_at='2026-09-01' WHERE id='attempt'").run();
  db.prepare("UPDATE pattern_report_attempts SET status='succeeded',finished_at='2026-09-01',html=?,html_hash='html-hash',metadata=? WHERE id='attempt'").run('<!DOCTYPE html><html><head></head><body>Historical report</body></html>','{"usage":{"cost":0.25}}');
  db.prepare("UPDATE pattern_reports SET selected_attempt_id='attempt' WHERE id='report'").run();
  const before=db.prepare('SELECT * FROM pattern_report_attempts').get();
  const exec=db.exec.bind(db);let inject=true;
  vi.spyOn(db,'exec').mockImplementation((sql:string)=>{if(inject&&sql.includes('DROP TABLE pattern_report_sources_v19')){exec(sql);throw new Error('injected v20 failure');}return exec(sql);});
  expect(()=>migrateDatabase(db,dir)).toThrow('injected v20 failure');expect(db.pragma('user_version',{simple:true})).toBe(19);
...
it('admits v6 at schema 21 without rewriting saved requests and recovers a failed admission', () => {
  ...
  const exec=db.exec.bind(db), fault=vi.spyOn(db,'exec').mockImplementation(sql=>{const value=exec(sql);if(sql.includes('Schema 21 admits'))throw new Error('v21 interruption');return value;});
  expect(()=>migrateDatabase(db,dir)).toThrow('v21 interruption');fault.mockRestore();
  ...
  migrateDatabase(reopened,dir);expect(reopened.pragma('user_version',{simple:true})).toBe(23);
...
it('admits eight-partner contracts from schema 22 without rewriting history and rolls back the new step', () => {
  ...
  expect(db.prepare('SELECT * FROM sessions').all()).toEqual(before);
  migrateDatabase(db,dir);expect(db.pragma('user_version',{simple:true})).toBe(23);
  expect(db.prepare('SELECT * FROM sessions').all()).toEqual(before);
  expect(db.prepare('SELECT document FROM shared_memory').pluck().get()).toBe(flatDocument);
});

it('upgrades the schema 21 start through flat-memory conversion and eight-partner admission',()=>{
  const {db,dir}=fixture(source21,21);db.transaction(()=>installCatalog19(db))();
  migrateDatabase(db,dir);expect(db.pragma('user_version',{simple:true})).toBe(23);validateSchema(db,current);
  expect(db.prepare('SELECT document FROM shared_memory').pluck().get()).toBe(flatDocument);
