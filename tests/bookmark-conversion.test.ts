...
beforeEach(() => {
  directory = mkdtempSync(join(tmpdir(), 'stomylos-bookmark-conversion-')); file = join(directory, 'stomylos.sqlite3');
  const db = new Database(file); db.exec(readFileSync('tests/fixtures/schema-v10-before-bookmarks.sql', 'utf8'));
  db.pragma('user_version=10'); new StarterStore(db).initialize();
...
it('preserves all original rows and private backup artifacts, then persists new marks across two opens', () => {
  ...
  for (const path of ['before-v10.sqlite3', 'verified-v11.sqlite3', 'schema-v10.sql', 'schema-v11.sql', 'manifest.json', 'convert-bookmarks.mjs'])
  ...
  for (let i = 0; i < 2; i++) {
    ...
    expect(store.view('old').bookmarked).toBe(i > 0); expect(store.view('old').canBookmark).toBe(true);
...
it('keeps prepared or failed-replacement sources exact and refuses changed acceptance hashes', () => {
  const before = readFileSync(file), report = convertBookmarks(file, { prepareOnly: true });
  expect(report.status).toBe('prepared'); expect(readFileSync(file)).toEqual(before);
  expect(() => convertBookmarks(file, { expectedSourceHash: '0'.repeat(64) })).toThrow('source_changed_since_acceptance');
  expect(() => convertBookmarks(file, { replace() { throw new Error('replace_failed'); } })).toThrow('replace_failed');
  expect(readFileSync(file)).toEqual(before);
  expect(() => convertBookmarks(file, { beforeReplace() {
  ...
  const db = new Database(file); expect(db.pragma('user_version', { simple: true })).toBe(10);
...
it.each(['version', 'structure', 'sidecar', 'backup'])('refuses an unsafe %s before replacing the source', failure => {
  ...
  const before = readFileSync(file); expect(() => convertBookmarks(file)).toThrow(); expect(readFileSync(file)).toEqual(before);
});
...
it('reproduces conversion from archived schemas with an explicitly supplied dependency runtime', () => {
  const report = convertBookmarks(file, { prepareOnly: true }), copy = join(directory, 'copy.sqlite3'); copyFileSync(file, copy);
  const result = spawnSync(process.execPath, [join(report.archive, 'convert-bookmarks.mjs'), copy], {
    env: { ...process.env, ELECTRON_RUN_AS_NODE: '1', STOMYLOS_CONVERTER_RUNTIME: resolve('.') }, encoding: 'utf8'
  });
  expect(result.status, result.stderr).toBe(0);
  const db = new Database(copy); expect(db.pragma('user_version', { simple: true })).toBe(11);
  expect(db.prepare('SELECT COUNT(*) n FROM session_bookmarks').get()).toEqual({ n: 0 }); db.close();
});
