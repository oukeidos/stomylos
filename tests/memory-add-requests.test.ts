...
it('renders each attempt and reported usage without inventing missing costs',()=>{
 const base={id:'a',job_id:7,input_number:1,message_id:'user',created_at:'2026-09-11T00:00:00Z',model:'openai/gpt-5.6-luna',reasoning:'none',failure:null};
 ...
 expect(html).toContain('Input 1');expect(html).not.toContain('Input 7');expect(html.match(/Memory update/g)).toHaveLength(2);expect(html).toContain('interrupted');expect(html).toContain('succeeded');expect(html).toContain('250 tokens');expect(html).toContain('30 output tokens');expect(html).toContain('1.2 seconds');expect(html).toContain('$0.00123');expect(html.match(/\$/g)).toHaveLength(1);
});
...
it.each(['request_timeout','transport_failed'])('shows uncertain billing for %s failures and omits it for known unsent attempts',failure=>{
 const row={id:'a',job_id:7,input_number:1,message_id:'m',created_at:'2026-09-11T00:00:00Z',model:'luna',reasoning:'none',status:'failed' as const,metadata:'{"elapsed_seconds":2.4}',failure};
