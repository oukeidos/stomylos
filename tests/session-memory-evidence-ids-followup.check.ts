// Explicit two-call evidence-ID probe; never included in ordinary npm test.
...
it('probes evidence message IDs for the authorized session exactly two additional times',async()=>{
 const directory='test-results/session-memory-evidence-ids-followup';mkdirSync(directory,{recursive:true,mode:0o700});
 ...
 expect(prepared.body).toEqual(JSON.parse(readFileSync('test-results/session-memory-evidence-ids/request.json','utf8')).body);
 ...
 writeFileSync(directory+'/started.json',JSON.stringify({at:new Date().toISOString(),calls:2,input_hash:saved.input_hash,body_hash:hash(JSON.stringify(prepared.body)),sentence}));
