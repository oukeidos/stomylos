...
it('rolls back raw COLD failure with HOT, provenance and completion, then applies the saved response after restart', () => {
  ...
  const attempt = receiveNotes(f.store, ['old'.repeat(800), 'new'.repeat(800)]);
  ...
  expect(new ColdMemoryStore(f.db).page().items[0].text).toBe('old'.repeat(800));
...
it('retains HOT and COLD provenance after chat deletion and archives a manual edit with its separate time basis', () => {
  const { store, db } = fixture(), session = coldSession(store);
  store.acceptMemoryAdd(receiveNotes(store, ['first'.repeat(500), 'second'.repeat(400)]).id);
  ...
  coldSession(store, 'More information.'); store.acceptMemoryAdd(receiveNotes(store, ['x'.repeat(2998)]).id);
...
it('explicit HOT deletion creates no archive, records revocation and original rows reject rewriting', () => {
  const { store, db } = fixture(), session = coldSession(store);
  store.acceptMemoryAdd(receiveNotes(store, ['old'.repeat(800), 'Retain then delete.']).id);
  store.end(session.id);
  ...
  coldSession(store); store.acceptMemoryAdd(receiveNotes(store, ['x'.repeat(2998)]).id);
...
it('searches literal Unicode text with NFC and case normalization without changing original bytes',()=>{
  ...
  const original='Cafe\u0301 100%_ 기억 😃';store.acceptMemoryAdd(receiveNotes(store,[original,'x'.repeat(2998)]).id);
