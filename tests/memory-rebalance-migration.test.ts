...
function fixture() {
  ...
  f.db.prepare('DELETE FROM session_memories WHERE session_id=?').run(session.id);
  f.db.prepare('INSERT INTO session_memories(session_id,character_id,document,document_hash) VALUES(?,?,?,?)').run(session.id,f.store.session(session.id).character,encoded,coldHash(encoded));
  f.store.close(); historicalSchema(f.db,34);
...
it('archives schema-34 overflow with provenance and a verified backup while retaining frozen chats and pending ADD', () => {
  ...
  expect(history(f.db)).toMatchObject(before);
  ...
  const next=f.store.createSession();f.store.selectManual(next.id,'model_03');
