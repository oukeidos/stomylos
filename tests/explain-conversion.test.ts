...
beforeEach(() => {
  directory = mkdtempSync(join(tmpdir(), 'stomylos-explain-conversion-')); file = join(directory, 'stomylos.sqlite3');
  const db = new Database(file); db.exec(readFileSync('tests/fixtures/schema-v12-before-explain.sql', 'utf8'));
  db.pragma('user_version=12'); new StarterStore(db).initialize();
...
it('preserves v12 rows, bookmarks, drafts and private backups across two v13 opens', () => {
  ...
  for (const path of ['before-v12.sqlite3', 'verified-v13.sqlite3', 'schema-v12.sql', 'schema-v13.sql', 'manifest.json', 'convert-explain.mjs'])
  ...
  for (let i = 0; i < 2; i++) {
    ...
    expect(store.explainList('old')).toEqual([]);
...
it('keeps prepared or failed-replacement sources exact and refuses changed acceptance hashes', () => {
  const before = readFileSync(file), report = convertExplain(file, { prepareOnly: true });
  expect(report.status).toBe('prepared'); expect(readFileSync(file)).toEqual(before);
  expect(() => convertExplain(file, { expectedSourceHash: '0'.repeat(64) })).toThrow('source_changed_since_acceptance');
  expect(() => convertExplain(file, { replace() { throw new Error('replace_failed'); } })).toThrow('replace_failed');
  expect(readFileSync(file)).toEqual(before);
  expect(() => convertExplain(file, { beforeReplace() {
  ...
  const db = new Database(file); expect(db.pragma('user_version', { simple: true })).toBe(12);
...
it.each(['version', 'structure', 'sidecar', 'backup'])('refuses an unsafe %s before replacing the source', failure => {
  ...
  const before = readFileSync(file); expect(() => convertExplain(file)).toThrow(); expect(readFileSync(file)).toEqual(before);
});
...
it('reproduces conversion from archived schemas with an explicitly supplied dependency runtime', () => {
  const report = convertExplain(file, { prepareOnly: true }), copy = join(directory, 'copy.sqlite3'); copyFileSync(file, copy);
  const result = spawnSync(process.execPath, [join(report.archive, 'convert-explain.mjs'), copy], {
    env: { ...process.env, ELECTRON_RUN_AS_NODE: '1', STOMYLOS_CONVERTER_RUNTIME: resolve('.') }, encoding: 'utf8'
  });
  expect(result.status, result.stderr).toBe(0);
  const db = new Database(copy); expect(db.pragma('user_version', { simple: true })).toBe(13);
