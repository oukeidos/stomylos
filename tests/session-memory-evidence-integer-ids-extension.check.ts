// Explicit seven-call integer evidence-ID probe; never included in ordinary npm test.
...
it('probes evidence message IDs for the authorized session with integer IDs exactly seven additional times',async()=>{
 const directory='test-results/session-memory-evidence-integer-ids-extension';mkdirSync(directory,{recursive:true,mode:0o700});
 ...
 input.conversation=input.conversation.map((m:any,i:number)=>({id:i+1,...m}));
 body.messages[1].content=JSON.stringify(input);
 body.response_format.json_schema.name='add_evidence_probe_v1';
 body.response_format.json_schema.schema.properties.add.items={type:'object',properties:{id:{type:'integer',minimum:1},text:{type:'string',minLength:1},evidence_ids:{type:'array',items:{type:'integer',enum:input.conversation.map((m:any)=>m.id)},minItems:1}},required:['id','text','evidence_ids'],additionalProperties:false};
 ...
 const priorBody=JSON.parse(readFileSync('test-results/session-memory-evidence-ids/request.json','utf8')).body;
 const normalized=structuredClone(prepared.body);
 const normalizedInput=JSON.parse(normalized.messages[1].content);
 normalizedInput.conversation=normalizedInput.conversation.map((m:any)=>({...m,id:`m${String(m.id).padStart(2,'0')}`}));
 normalized.messages[1].content=JSON.stringify(normalizedInput);
 normalized.response_format.json_schema.schema.properties.add.items.properties.evidence_ids.items=priorBody.response_format.json_schema.schema.properties.add.items.properties.evidence_ids.items;
 expect(normalized).toEqual(priorBody);
