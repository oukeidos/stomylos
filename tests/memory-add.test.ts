...
it.each([
 ...
 ['2026-09-12T01:30:00.000Z',-420,'2026-09-11 (Friday)'],
 ['2024-02-29T12:00:00.000Z',0,'2024-02-29 (Thursday)'],
])('projects ADD local dates without changing recorded time: %s', (utc,offset,expected)=>{
 const sent=recordedTime(utc,null,offset),input={timezone:null,current_user:{content:'Yesterday, it worked.',sent_at:utc},previous_assistant:{content:'The switch?'}};
 const before=JSON.stringify({sent,input});
 expect(JSON.parse(memoryAddBody(input,sent).messages[1].content)).toEqual({current_user:{content:input.current_user.content,sent_at:expected},previous_assistant:input.previous_assistant});
 expect(JSON.stringify({sent,input})).toBe(before);
});
it('uses null for unknown or invalid ADD calendar dates without guessing from UTC',()=>{
 for(const local_date of ['', '2026-02-30', 'not-a-date', '2026-9-11']){
  expect(memoryAdd.memoryAddDate({...recordedTime('2026-09-11T12:00:00.000Z',null,0),local_date})).toBeNull();
 }
 expect(memoryAdd.memoryAddDate(null)).toBeNull();expect(memoryAdd.memoryAddDate()).toBeNull();
 expect(JSON.parse(memoryAddBody({timezone:'UTC',current_user:{content:'Tomorrow.',sent_at:'2026-09-11T12:00:00Z'}}).messages[1].content)).toEqual({current_user:{content:'Tomorrow.',sent_at:null}});
});
it('freezes L wire dates while retaining full source times and effective provider policy',()=>{
 ...
 expect(JSON.parse(savedConfig).version).toBe('stomylos_memory_add_v2');
 const a=prepare(f.store),body=JSON.parse(a.body),wire=JSON.parse(body.messages[1].content);
 expect(wire).not.toHaveProperty('timezone');expect(wire.current_user).toEqual({content:'Yesterday, I fixed it.',sent_at:'2026-09-12 (Saturday)'});
 expect(body.messages[0].content).toBe(readFileSync('tests/fixtures/memory-add/merge-only-l-prompt.txt','utf8'));
 const effective=JSON.parse(f.db.prepare('SELECT provider_request FROM memory_add_attempts WHERE id=?').pluck().get(a.id) as string);
 expect(JSON.stringify(effective)).toContain('Saturday');
 expect(JSON.stringify(effective)).toContain('"data_collection":"deny"');
 expect(JSON.stringify(effective)).toContain('"allow_fallbacks":true');
 expect(f.db.prepare('SELECT sent_at_utc,timezone,utc_offset_minutes FROM message_times WHERE message_id=?').get(job.message_id)).toEqual({sent_at_utc:sent.utc,timezone:sent.timezone,utc_offset_minutes:540});
...
it('does not auto-retry unknown calls, blocks later sources, and rejects stale attempt commits after exact-job retry/skip',()=>{
 ...
 const oldPrompt=readFileSync('tests/fixtures/memory-add/grouping-prompt.txt','utf8'),currentBuilder=memoryAddBody;
 const legacy=vi.spyOn(memoryAdd,'memoryAddBody').mockImplementationOnce(input=>{const body=currentBuilder(input);body.messages[0].content=oldPrompt;body.messages[1].content=JSON.stringify(input);return body;});
 const legacyVersion=vi.spyOn(memoryAdd,'memoryAddRequestVersion','get').mockReturnValue('stomylos_memory_add_v1' as typeof memoryAdd.memoryAddRequestVersion);
 let start:ReturnType<typeof send>;
 try{start=send(f.store,s.id);}finally{legacy.mockRestore();legacyVersion.mockRestore();}
 ...
 expect(JSON.parse(JSON.parse(b.body).messages[1].content).current_user.sent_at).toContain('T');
...
expect(f.store.memoryAddReady()?.ordinal).toBe(2);
 ...
 expect(JSON.parse(JSON.parse(c.body).messages[1].content)).not.toHaveProperty('timezone');
 expect(JSON.parse(JSON.parse(c.body).messages[1].content).current_user.sent_at).toMatch(/^\d{4}-\d{2}-\d{2} \([A-Za-z]+\)$/);
});
...
it('runs ADD before a held reply with a frozen baseline, sends one request per input, and drains End without Gemini or cleanup',async()=>{
 ...
 let releaseMemory!:()=>void;const heldMemory=new Promise<void>(r=>releaseMemory=r);
 const gateway:Gateway={async complete(body,identity){calls.push(body);await heldMemory;expect(identity.allowed_models).toContain(body.model);return {content:'{"add":["Likes tea."]}',metadata:{usage:{cost:0.001}}};},async stream(body,_signal,chunk){replies.push(body);await held;chunk('Tell me more.');return {content:'Tell me more.',metadata:{}};}};
 const c=new Coordinator(client,gateway,{keyPresent:true,keyPath:'',dataPath:f.dir,appVersion:'test',development:true},()=>{},()=>true);
 try{await c.command('sendMessage',{sessionId:s.id,text:'I like tea.',revision:0});
 await vi.waitFor(()=>expect(calls).toHaveLength(1));expect((await c.snapshot()).activity.memoryProcessing).toBe(true);releaseMemory();
 await vi.waitFor(()=>expect(f.store.currentMemory().revision).toBe(1));
 ...
 await vi.waitFor(async()=>expect((await c.snapshot()).activity.memoryProcessing).toBe(false));
 ...
 }finally{releaseMemory();release();await c.command('close',undefined);}
});
