...
it('keeps the ordered definition contract and frozen historical display identities',()=>{
  ...
  for(const body of [routerBody(null,'Hello',conversationSnapshot('user')),routerBody('Question','Answer',conversationSnapshot('starter')),f.body]){
    expect(body.messages[0].content.match(/model_\d+/g)).toEqual(partnerOrder.filter(id => !['model_04','model_08'].includes(id)));
    expect(Object.keys(body.response_format.json_schema.schema.properties)).toEqual(partnerOrder.filter(id => !['model_04','model_08'].includes(id)));
  }
