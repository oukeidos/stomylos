...
import old from '../src/main/conversation-v9-config.json';
...
const prior = (kind: 'user'|'starter'): Json => ({...conversationSnapshot(kind),...structuredClone(old.conversation),router_prompt_version:'stomylos_compact_router_v3'});
it('removes DeepSeek Pro from new manual and every Auto entry while preserving all remaining cards', () => {
  const current=conversationSnapshot('user'), ids=current.characters.map((c:Json)=>c.id);
  expect(current.characters).toEqual(old.conversation.characters.filter(c=>c.id!=='model_08'));
  ...
  for (const kind of ['user','starter','opener'] as const) {
    ...
    expect(b.messages[0].content).not.toContain('model_08');
  ...
  expect(partnerRouterBody(route).messages[0].content).not.toContain('model_08');validateRecovery(route);
  const scores=Object.fromEntries(ids.map((id:string)=>[id,1]));
  expect(()=>routerScores(JSON.stringify({...scores,model_08:2}),current)).toThrow();
  expect(eligible(null,current)).toEqual(['model_02','model_09']);
  expect(chooseOtherPartner(current,current.characters[0].model,scores,{}).pool).not.toContain('model_08');
  for(const kind of ['user','starter'] as const){const s=prior(kind); const r=routerSnapshot(s);validateRecovery(r);validateRecovery(recoverySnapshot(r));expect(r.parameters.response_format.json_schema.schema.required).toContain('model_08');}
  const historicalRoute=partnerRouterSnapshot(prior('user'),messages,old.conversation.characters[0].model);
  validateRecovery(historicalRoute);expect(partnerRouterBody(historicalRoute).messages[0].content).toContain('model_08');
});
it('preserves v9 DeepSeek Pro history and exact failed request through admission, reopen and retry; new sessions reject DeepSeek Pro', () => {
  ...
  store.searchMode(session.id,'off');store.selectManual(session.id,'model_08');store.submit(session.id,'An ordinary bicycle conversation.');
  ...
  expect(partnerDisplayName(saved.characters.find((c:Json)=>c.id==='model_08'))).toBe('Prefer · DeepSeek V4 Pro 0813');
  ...
  const fresh=store.createSession();expect(()=>store.selectManual(fresh.id,'model_08')).toThrow('invalid_character');
  expect(JSON.parse(fresh.chat_config).characters.map((c:Json)=>c.id)).not.toContain('model_08');
});
