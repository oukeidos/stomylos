...
const prepared = prepareProviderRequest({ model: 'selected' }, { allowed_models: ['selected'], provider: 'OpenAI' });
...
it('decodes split UTF-8 bytes and sets bounded nonredirecting HTTP options', async () => {
  ...
  expect((await gateway.complete(body, identity!, signal(), 10_000)).content).toBe('안녕 🌳');
...
it('handles fragmented SSE UTF-8 and rejects a response above the byte cap', async () => {
  ...
  expect((await gateway.stream(body, signal(), text => seen.push(text))).content).toBe('안녕 🌳');
  expect(seen).toEqual(['안녕 🌳']);
  vi.stubGlobal('fetch', async () => bytesResponse('x'.repeat(2 * 1024 * 1024 + 1), 32768));
  await expect(gateway.complete(body, identity!, signal(), 10_000)).rejects.toThrow('response_too_large');
});
it('returns redacted HTTP categories and rejects malformed UTF-8', async () => {
  ...
  await expect(gateway.complete(body, identity!, signal(), 10_000)).rejects.toThrow(/^http_401$/);
  vi.stubGlobal('fetch', async () => new Response(new Uint8Array([0xff, 0xfe])));
  await expect(gateway.complete(body, identity!, signal(), 10_000)).rejects.toThrow(/^transport_failed$/);
});
it('distinguishes total timeout, idle timeout and explicit cancellation', async () => {
  ...
  const gateway = new OpenRouter(() => 'dummy');
  const total = expect(gateway.complete(body, identity!, signal(), 10_000)).rejects.toThrow('request_timeout');
  await vi.advanceTimersByTimeAsync(10_001); await total;
  const idle = expect(gateway.stream(body, signal(), () => undefined)).rejects.toThrow('stream_idle_timeout');
  await vi.advanceTimersByTimeAsync(30_001); await idle;
  const abort = new AbortController(); const cancelled = expect(gateway.complete(body, identity!, abort.signal, 10_000)).rejects.toThrow('request_cancelled');
...
it('uses the shorter search-gate deadline and adds metadata headers only for search paths', async () => {
  ...
  const pending = expect(new OpenRouter(() => 'dummy').stream(body, signal(), () => undefined,
...
it('permits the larger response bound only for search-enabled chat and still enforces eight million bytes', async () => {
  ...
  const gateway = new OpenRouter(() => 'dummy');
  await expect(gateway.stream(body, signal(), () => undefined)).rejects.toThrow('response_too_large');
  expect((await gateway.stream(body, signal(), () => undefined, { search: true })).content).toBe('Answer');
  vi.stubGlobal('fetch', async () => bytesResponse(':' + 'x'.repeat(8000000), 65536));
  await expect(gateway.stream(body, signal(), () => undefined, { search: true })).rejects.toThrow('response_too_large');
});
...
it('preserves only visible response and safe usage when a complete response has the wrong identity', async () => {
  ...
  const failed = await new OpenRouter(() => 'dummy').complete(body, identity!, signal(), 1000).catch(error => error);
...
it.each(['length', 'content_filter', 'error'])('retains the %s stream finish and trailing usage without reasoning text', async reason => {
  ...
  const failure = await new OpenRouter(() => 'dummy').stream(body, signal(), () => undefined).catch(e => e);
