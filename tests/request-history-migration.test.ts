...
function fixture(){
 ...
 const db=new Database(join(dir,'stomylos.sqlite3'));dbs.push(db);historicalSchema(db,33);
...
it('upgrades schema 33 with immutable history, verified backup, empty new history and current no-op',()=>{
 ...
 expect(db.prepare('SELECT * FROM sessions').all()).toMatchObject(before);expect(db.prepare('SELECT * FROM genie_request_attempts').all()).toEqual([]);
...
it('rolls back the new history step and resumes without replacing the original recovery backup',()=>{
 ...
 expect(db.pragma('user_version',{simple:true})).toBe(33);expect(db.prepare('SELECT * FROM sessions').all()).toMatchObject(before);
