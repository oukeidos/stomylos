// Explicitly authorized two-session 40-call repeat probe; excluded from ordinary tests.
...
const directory='test-results/session-memory-severe-repeat-extension';
it('repeats the two selected sessions ten more times per setting',async()=>{
 ...
 const tasks:any[]=[];
 for(let run=11;run<=20;run++) for(const label of ['S07','S25']) {
 ...
 await Promise.all([worker(),worker()]);expect(results).toHaveLength(40);
},4000000);
