// Ten authorized source-location probes; not content validation or production work.
...
it('locates sources for five samples with two cheap models',async()=>{
 const dir='test-results/session-record-source-cheap';mkdirSync(dir,{recursive:true,mode:0o700});
 ...
 const configs=[{model:'google/gemma-4-31b-it',reasoning:{enabled:false,exclude:true}},{model:'openai/gpt-oss-120b',reasoning:{effort:'low',exclude:true}}];const tasks:any[]=[];
 for(const label of ['S01','S08','S42','S45','S49']){
  const s=sessions.find((s:any)=>s.label===label);const records=outputs.find((r:any)=>r.label===label&&r.variant==='original').records;
  ...
  for(const c of configs){const body={model:c.model,reasoning:c.reasoning,max_tokens:4096,messages:[{role:'system',content:prompt},{role:'user',content:JSON.stringify(input)}],response_format:{type:'json_schema',json_schema:{name:'record_sources',strict:true,schema}},provider:{require_parameters:true}};const p=prepareProviderRequest(body,{allowed_models:[c.model],provider:null});tasks.push({label,model:c.model,input,body:p.body,identity:p.identity});}
 }
 ...
 closeSync(openSync(dir+'/started.json','wx',0o600));writeFileSync(dir+'/started.json',JSON.stringify({calls:10,at:new Date().toISOString(),hash:hash(JSON.stringify(tasks))}),{mode:0o600});writeFileSync(dir+'/requests.json',JSON.stringify(tasks,null,2),{mode:0o600});
 ...
 r.elapsed_seconds=(performance.now()-start)/1000;save();}}
 await Promise.all([worker(),worker()]);expect(results).toHaveLength(10);
},1200000);
