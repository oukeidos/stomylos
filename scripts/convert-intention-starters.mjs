// Standalone, explicitly invoked v9 -> v10 conversion. Never imported by the app.
...
export function convertIntentionStarters(file, options = {}) {
  ...
  try {
    ...
    const frozen = existsSync(join(scriptDirectory, 'schema-v9.sql'));
    const oldSchema = readFileSync(frozen ? join(scriptDirectory, 'schema-v9.sql') : join(root, 'tests/fixtures/schema-v9-before-intentions.sql'), 'utf8');
    const newSchema = readFileSync(frozen ? join(scriptDirectory, 'schema-v10.sql') : join(root, 'tests/fixtures/schema-v10-before-bookmarks.sql'), 'utf8');
    ...
    source = new Database(file, { readonly: true, fileMustExist: true }); check(source, expectedOld, 9);
    ...
    archive = join(backups, 'intention-starters-v10-' + randomUUID()); mkdirSync(archive, { mode: 0o700 });
    const backup = join(archive, 'before-v9.sqlite3'), converted = join(archive, 'verified-v10.sqlite3');
    ...
    target = new Database(converted); chmodSync(converted, 0o600);
    target.pragma('foreign_keys=OFF'); target.pragma('synchronous=FULL');
    target.transaction(() => {
      target.exec(newSchema);
      ...
      try {
        for (const [name, data] of Object.entries(originalRows)) {
          if (name === 'sqlite_sequence') continue;
          target.prepare('DELETE FROM ' + quote(name)).run();
        ...
        // Preserve AUTOINCREMENT high-water marks even when the last row was deleted.
        if (originalRows.sqlite_sequence) {
          target.prepare('DELETE FROM sqlite_sequence').run();
          for (const row of originalRows.sqlite_sequence) target.prepare('INSERT INTO sqlite_sequence(rowid,name,seq) VALUES(?,?,?)').run(...row);
        }
      } finally { backupDb.close(); }
      for (const trigger of triggers) target.exec(trigger.sql);
      target.pragma('user_version=10');
    })();
    target.pragma('foreign_keys=ON'); check(target, expectedNew, 10);
    const backupDb = new Database(backup, { readonly: true, fileMustExist: true });
    try {
      ...
      for (const name of ['starter_preparations','intention_question_state','intention_question_jobs','intention_question_attempts']) {
    ...
    writeFileSync(join(archive, 'schema-v9.sql'), oldSchema, { mode: 0o600, flag: 'wx', flush: true });
    writeFileSync(join(archive, 'schema-v10.sql'), newSchema, { mode: 0o600, flag: 'wx', flush: true });
    copyFileSync(fileURLToPath(import.meta.url), join(archive, 'convert-intention-starters.mjs'), constants.COPYFILE_EXCL);
    manifest = { status: 'prepared', source_version: 9, target_version: 10, source: file, archive,
      ...
      schemas: { v9: digest(join(archive, 'schema-v9.sql')), v10: digest(join(archive, 'schema-v10.sql')) } };
    ...
    for (const suffix of ['-wal', '-shm', '-journal']) if (existsSync(file + suffix) && lstatSync(file + suffix).size) throw new Error('unclean_sidecar');
    const staged = join(directory, '.intention-starters-v10-' + randomUUID() + '.sqlite3');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!file) throw new Error('Usage: ELECTRON_RUN_AS_NODE=1 electron scripts/convert-intention-starters.mjs /absolute/path/stomylos.sqlite3 [--prepare-only] [--expected-source-sha256 HASH]');
  writeSync(1, JSON.stringify(convertIntentionStarters(file, options), null, 2) + '\n');
}
