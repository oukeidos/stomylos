...
const directory = resolve('test-results/model-switching-copy');
...
assert inventory(source)['version']==11
...
`, source, file);
const conversion = JSON.parse(execFileSync(require('electron'), ['scripts/convert-model-switching.mjs', file, '--expected-source-sha256', prepared.source_sha256], {
  env: { ...process.env, ELECTRON_RUN_AS_NODE: '1' }, encoding: 'utf8'
}));
const inventory = path => py('print(json.dumps(inventory(sys.argv[1])))', path);
const before = inventory(file); assert.equal(before.version, 12);
for (const [name, rows] of Object.entries(prepared.baseline.tables)) assert.deepEqual(before.tables[name], rows, name);
for (const name of ['session_partner_state','partner_selection_operations']) assert.equal(before.tables[name].count, 0);
const env = { ...process.env, HOME: mkdtempSync(join(tmpdir(), 'stomylos-partner-copy-home-')), STOMYLOS_DATA_DIR: directory };
...
const executablePath = resolve(process.env.STOMYLOS_VERIFY_BUNDLE ?? 'release/model-switch-candidate/linux-unpacked/stomylos');
...
try {
  for (let i = 0; i < 2; i++) {
    ...
    assert.equal(result.version, '0.19.0'); assert.equal(result.keyPresent, false); opens.push(result);
  ...
  writeFileSync('test-results/model-switching-copy-acceptance.json', JSON.stringify(report, null, 2), { mode: 0o600 });
