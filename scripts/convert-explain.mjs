// Standalone, explicitly invoked v12 -> v13 conversion. Never imported by the app.
...
export function convertExplain(file, options = {}) {
  ...
  try {
    ...
    source = new Database(file, { readonly: true, fileMustExist: true }); check(source, expectedOld, 12);
    ...
    archive = join(backups, 'explain-v13-' + randomUUID()); mkdirSync(archive, { mode: 0o700 });
    const backup = join(archive, 'before-v12.sqlite3'), converted = join(archive, 'verified-v13.sqlite3');
    ...
    target.transaction(() => {
      ...
      target.pragma('user_version=13');
    })();
    target.pragma('foreign_keys=ON'); check(target, expectedNew, 13);
    ...
    try {
      ...
      for (const name of ['explanations', 'explanation_attempts']) {
    ...
    writeFileSync(join(archive, 'schema-v12.sql'), oldSchema, { mode: 0o600, flag: 'wx', flush: true });
    writeFileSync(join(archive, 'schema-v13.sql'), newSchema, { mode: 0o600, flag: 'wx', flush: true });
    copyFileSync(fileURLToPath(import.meta.url), join(archive, 'convert-explain.mjs'), constants.COPYFILE_EXCL);
    chmodSync(join(archive, 'convert-explain.mjs'), 0o600); sync(join(archive, 'convert-explain.mjs'));
    manifest = { status: 'prepared', source_version: 12, target_version: 13, source: file, archive,
      ...
      schemas: { v12: digest(join(archive, 'schema-v12.sql')), v13: digest(join(archive, 'schema-v13.sql')) } };
    ...
    for (const suffix of ['-wal', '-shm', '-journal']) if (existsSync(file + suffix) && lstatSync(file + suffix).size) throw new Error('unclean_sidecar');
    const staged = join(directory, '.explain-v13-' + randomUUID() + '.sqlite3');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!file) throw new Error('Usage: ELECTRON_RUN_AS_NODE=1 electron scripts/convert-explain.mjs /absolute/path/stomylos.sqlite3 [--prepare-only] [--expected-source-sha256 HASH]');
  writeSync(1, JSON.stringify(convertExplain(file, options), null, 2) + '\n');
}
