...
try {
  ...
  await page.keyboard.press('Escape'); await button('Hyphantes').click(); await button('Close Hyphantes').waitFor(); await assert.rejects(()=>command('explainHistory',{sessionId,messageId}),/genie_busy/); await button('Close Hyphantes').click(); await page.locator('.genie-dock').waitFor({state:'detached'}); await root().focus(); await page.evaluate(()=>new Promise(requestAnimationFrame));
  report.checks.push('Local-save failure keeps result visible; save retry makes no model call; Hyphantes exclusion in both directions');
