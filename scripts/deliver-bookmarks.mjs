...
const candidate = resolve('release/bookmark-candidate/linux-unpacked'), active = resolve('release/linux-unpacked');
const reportPath = resolve('test-results/bookmark-delivery.json');
const previousDelivery = read('test-results/intention-delivery.json');
const source = previousDelivery.source;
const gate = read('test-results/bookmark-package-parity.json');
assert.equal(gate.status, 'passed');
for (const name of ['bookmarks-native/report.json','bookmarks-packaged/report.json']) assert.equal(read('test-results/' + name).status, 'passed');
...
assert.equal(JSON.parse(extractFile(candidateAsar, 'package.json')).version, '0.18.0');
assert.equal(JSON.parse(extractFile(join(active, 'resources/app.asar'), 'package.json')).version, '0.17.0');
...
baseline=inventory(source);assert baseline['version']==10
folder=root/'test-results'/('bookmark-data-copy-'+str(uuid.uuid4()));folder.mkdir(mode=0o700)
...
const converted = JSON.parse(execFileSync(runtime, ['scripts/convert-bookmarks.mjs', prepared.copy, '--expected-source-sha256', prepared.sourceSha256],
...
for (const name of ['session_bookmarks']) assert.equal(before.tables[name].count, 0);
const cleanedEnv = () => {
  ...
  try {
    ...
    const result = await page.evaluate(async ids => {
      ...
      return { version: snapshot.settings.appVersion, dataPath: snapshot.settings.dataPath, development: snapshot.settings.development,
        ...
        eligibleIds: views.filter(v => v.canBookmark).map(v => v.session.id),
        bookmarkedIds: views.filter(v => v.bookmarked).map(v => v.session.id) };
    }, expected.sessions);
    assert.equal(result.version, '0.18.0'); assert.equal(result.development, !normal); assert.equal(result.simulation, false);
    ...
    assert.equal(result.memoryItems, expected.memoryItems);
    if (!normal) {
      const id = result.eligibleIds[0]; assert.ok(id, 'Copy must contain an eligible historical chat');
      const desired = !result.bookmarkedIds.includes(id);
      await page.evaluate(async ({ sessionId, bookmarked }) => {
        await window.stomylos.command('setSessionBookmark', { sessionId, bookmarked });
        await window.stomylos.command('setSessionBookmark', { sessionId, bookmarked });
      }, { sessionId: id, bookmarked: desired });
      const view = await page.evaluate(sessionId => window.stomylos.command('loadSession', { sessionId }), id);
      assert.equal(view.bookmarked, desired);
      result.bookmarkExercise = { sessionId: id, initiallyMarked: !desired, committedMarked: desired };
    }
...
for (let i=0;i<2;i++) {
  ...
  assert.deepEqual(py('print(json.dumps(inventory(sys.argv[1],json.loads(sys.argv[2]))))', prepared.copy, JSON.stringify(columns)).tables,
    prepared.baseline.tables, 'Copy bookmark exercise changed original records');
}
assert.equal(copyPasses[0].bookmarkExercise.initiallyMarked, false);
assert.equal(copyPasses[1].bookmarkExercise.initiallyMarked, true);
assert.deepEqual(py('print(json.dumps(inventory(sys.argv[1])))', prepared.copy), before, 'Copy mark/unmark did not restore the empty bookmark table');
...
writeFileSync('test-results/bookmark-copy-acceptance.json', JSON.stringify(acceptance, null, 2) + '\n', { mode: 0o600, flush: true });
...
root=Path.cwd();a=json.loads((root/'test-results/bookmark-copy-acceptance.json').read_text());source=Path(a['source']);data=source.parent
...
candidate=root/'release/bookmark-candidate/linux-unpacked';active=root/'release/linux-unpacked'
def files(folder):
 result={}
...
ident=str(uuid.uuid4());backup=data/'backups'/('bookmark-delivery-'+ident);backup.mkdir(mode=0o700)
shutil.copy2(source,backup/'before-v10.sqlite3');os.chmod(backup/'before-v10.sqlite3',0o600);assert sha(backup/'before-v10.sqlite3')==a['sourceSha256']
...
shutil.copy2(root/'scripts/deliver-bookmarks.mjs',backup/'deliver-bookmarks.mjs')
stage=root/'release'/('.bookmark-stage-'+ident);shutil.copytree(candidate,stage);assert files(stage)==selected
staged_db=data/('.bookmark-v11-'+ident+'.sqlite3');shutil.copy2(converted,staged_db);os.chmod(staged_db,0o600)
assert inventory(staged_db)==a['converted']
old=root/'release'/('before-bookmark-0.17.0-'+ident)
report={'status':'backed_up','backup':str(backup),'source':str(source),'candidate':str(candidate),'active':str(active),'priorBundle':str(old),
 ...
 'rollback':'Preserve newer data first. A matched pre-delivery 0.17.0/v10 restore is in priorBundle and before-v10.sqlite3; prefer forward repair after v11 writes.'}
def save():
 for p in [backup/'delivery.json',root/'test-results/bookmark-delivery.json']:
...
try:
 ...
 if swapped:
  rollback=data/('.bookmark-rollback-'+ident+'.sqlite3');shutil.copy2(backup/'before-v10.sqlite3',rollback);os.replace(rollback,source)
...
`);
  ...
  console.log(JSON.stringify({ status: report.status, version: '0.18.0', schema: 11, sessions: before.sessions.length, memoryItems: before.memoryItems, opens: report.normalLaunches.length, backup: report.backup, modelRequests: 0 }));
}
