// Standalone, explicitly invoked v6 -> v7 conversion. Never imported by the app.
...
export function convertPatterns(file, options = {}) {
  ...
  try {
    ...
    const frozen = existsSync(join(scriptDirectory, 'schema-v6.sql'));
    const oldSchema = readFileSync(frozen ? join(scriptDirectory, 'schema-v6.sql') : join(root, 'tests/fixtures/schema-v6-before-pattern.sql'), 'utf8');
    const newSchema = readFileSync(frozen ? join(scriptDirectory, 'schema-v7.sql') : join(root, 'tests/fixtures/schema-v7-before-search.sql'), 'utf8');
    ...
    source = new Database(file, { readonly: true, fileMustExist: true }); check(source, expectedOld, 6);
    ...
    archive = join(backups, 'patterns-v7-' + randomUUID()); mkdirSync(archive, { mode: 0o700 });
    const backup = join(archive, 'before-v6.sqlite3'), converted = join(archive, 'verified-v7.sqlite3');
    ...
    target.transaction(() => { target.exec(newSchema.slice(oldSchema.length)); target.pragma('user_version=7'); })();
    check(target, expectedNew, 7);
    ...
    for (const name of ['pattern_reports', 'pattern_report_sources', 'pattern_report_attempts']) if (convertedRows[name].length) throw new Error('unexpected_backfill');
    target.close(); target = null; sync(converted);
    writeFileSync(join(archive, 'schema-v6.sql'), oldSchema, { mode: 0o600, flag: 'wx', flush: true });
    writeFileSync(join(archive, 'schema-v7.sql'), newSchema, { mode: 0o600, flag: 'wx', flush: true });
    copyFileSync(fileURLToPath(import.meta.url), join(archive, 'convert-patterns.mjs'), constants.COPYFILE_EXCL);
    manifest = { status: 'prepared', source_version: 6, target_version: 7, source: file, archive,
      ...
      schemas: { v6: digest(join(archive, 'schema-v6.sql')), v7: digest(join(archive, 'schema-v7.sql')) } };
    ...
    for (const suffix of ['-wal', '-shm', '-journal']) if (existsSync(file + suffix) && lstatSync(file + suffix).size) throw new Error('unclean_sidecar');
    const staged = join(directory, '.patterns-v7-' + randomUUID() + '.sqlite3');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!file) throw new Error('Usage: ELECTRON_RUN_AS_NODE=1 electron scripts/convert-patterns.mjs /absolute/path/stomylos.sqlite3 [--prepare-only] [--expected-source-sha256 HASH]');
  writeSync(1, JSON.stringify(convertPatterns(file, options), null, 2) + '\n');
}
