...
try {
  ...
  const panel = page.getByRole('region',{name:'End processing'});
  await panel.waitFor(); assert.match(await panel.innerText(),/Memory cleanup: failed/);
  ...
  await page.getByText('End processing complete',{exact:true}).waitFor();
