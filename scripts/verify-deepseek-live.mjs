...
const prepare = process.argv.includes('--prepare'), mock = process.argv.includes('--mock'), live = process.argv.includes('--authorize-36-calls-usd5');
if (!directory || !isAbsolute(directory) || [prepare, mock, live].filter(Boolean).length !== 1) throw Error('Use exactly --prepare, --mock or --authorize-36-calls-usd5 with --run-dir /absolute/path.');
...
if (prepare || mock) {
  ...
  const fixturePath = join(root, 'tests/fixtures/deepseek-live-cases.json');
  const selectionPath = resolve(root, '../auxiliary/conversation-model-evaluation/selected-conversation-models.json');
  ...
  const cases = read(fixturePath); assert.equal(cases.router_cases.length, 14); assert.equal(cases.chat_turns.length, 3); assert.equal(cases.memory_cases.length, 0);
  const selected = read(selectionPath).models; assert.equal(selected.length, 7);
  save('gate.json', { mock, maximumCalls: 36, maximumUsd: 5, maximumSearchExecutions: 2, frozenAt: new Date().toISOString(),
    selected, cases, hashes: Object.fromEntries(paths.map(path => [path, digest(readFileSync(path))])),
    scope: 'Exact candidate v7 assembly, 28 seven-model character routes, three DeepSeek Off conversation replies, two DeepSeek matched Auto/Off replies and at most two search-router attempts. Shared memory/time wrappers included; no changed updater. No automatic retry, normal user data or product writes. Operational results require separate content review.' }, true);
  if (prepare) { console.log(JSON.stringify({ status: 'prepared', directory, maximumCalls: 36, maximumUsd: 5, paidRequests: 0 })); process.exit(0); }
}
const gate = read(join(directory, 'gate.json'));
assert.equal(gate.mock, mock); assert.equal(gate.maximumCalls, 36); assert.equal(gate.maximumUsd, 5);
...
const prices = {}, modelIds = [...new Set([...lib.characters.filter(c => c.id === 'model_08').map(c => c.model), lib.routerSnapshot().parameters.model, ...lib.searchSnapshot().models.map(m => m.model)])];
...
async function dispatch(body, execute) {
  ...
  if (fatal || ledger.length >= 36 || held + reservation > 5) { fatal = true; throw Error('Gate budget exhausted before dispatch.'); }
...
function assembled(partner, messages) {
  const snapshot = lib.conversationSnapshot('user'); snapshot.memory_context = lib.emptyMemory('shared');
...
try {
  ...
  for (const partner of lib.characters.filter(c => c.id === 'model_08')) {
    const messages = [];
  ...
  for (const partner of lib.characters.filter(c => c.id === 'model_08')) for (const mode of ['auto', 'off']) await check(`search-${partner.id}-${mode}`, async () => {
...
if (mock) { assert.equal(ledger.length, 34); assert.equal(report.results.length, 33); assert.equal(report.status, 'operational_pass_review_pending'); }
