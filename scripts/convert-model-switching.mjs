// Standalone, explicitly invoked v11 -> v12 conversion. Never imported by the app.
...
export function convertModelSwitching(file, options = {}) {
  ...
  try {
    ...
    const frozen = existsSync(join(scriptDirectory, 'schema-v11.sql'));
    const oldSchema = readFileSync(frozen ? join(scriptDirectory, 'schema-v11.sql') : join(root, 'tests/fixtures/schema-v11-before-model-switching.sql'), 'utf8');
    const newSchema = readFileSync(frozen ? join(scriptDirectory, 'schema-v12.sql') : join(root, 'tests/fixtures/schema-v12-before-explain.sql'), 'utf8');
    ...
    source = new Database(file, { readonly: true, fileMustExist: true }); check(source, expectedOld, 11);
    ...
    archive = join(backups, 'model-switching-v12-' + randomUUID()); mkdirSync(archive, { mode: 0o700 });
    const backup = join(archive, 'before-v11.sqlite3'), converted = join(archive, 'verified-v12.sqlite3');
    ...
    target.transaction(() => {
      ...
      target.pragma('user_version=12');
    })();
    target.pragma('foreign_keys=ON'); check(target, expectedNew, 12);
    ...
    try {
      ...
      for (const name of ['session_partner_state', 'partner_selection_operations']) {
    ...
    writeFileSync(join(archive, 'schema-v11.sql'), oldSchema, { mode: 0o600, flag: 'wx', flush: true });
    writeFileSync(join(archive, 'schema-v12.sql'), newSchema, { mode: 0o600, flag: 'wx', flush: true });
    copyFileSync(fileURLToPath(import.meta.url), join(archive, 'convert-model-switching.mjs'), constants.COPYFILE_EXCL);
    chmodSync(join(archive, 'convert-model-switching.mjs'), 0o600); sync(join(archive, 'convert-model-switching.mjs'));
    manifest = { status: 'prepared', source_version: 11, target_version: 12, source: file, archive,
      ...
      schemas: { v11: digest(join(archive, 'schema-v11.sql')), v12: digest(join(archive, 'schema-v12.sql')) } };
    ...
    for (const suffix of ['-wal', '-shm', '-journal']) if (existsSync(file + suffix) && lstatSync(file + suffix).size) throw new Error('unclean_sidecar');
    const staged = join(directory, '.model-switching-v12-' + randomUUID() + '.sqlite3');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!file) throw new Error('Usage: ELECTRON_RUN_AS_NODE=1 electron scripts/convert-model-switching.mjs /absolute/path/stomylos.sqlite3 [--prepare-only] [--expected-source-sha256 HASH]');
  writeSync(1, JSON.stringify(convertModelSwitching(file, options), null, 2) + '\n');
}
