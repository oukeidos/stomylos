...
import { compactRouterPrompts, compactRouterVersion, eightRouterPrompts, eightRouterVersion, retainedRouterPrompts, retainedRouterVersion, currentRouterPrompts, currentRouterVersion } from './compact-router';
...
export function verifyRuntime() {
  for (const [text, expected] of [
    [currentRouterPrompts.direct, '2c4a4b3f812c3d4c62b0aa4a07af305cebb2c5af031d517901d061eae541953c'],
    [currentRouterPrompts.starter, '342a01eb188c23561a17becb93cbba97a57a866c8246848499b54464c30f9713'],
    [currentRouterPrompts.opener, '4b3ad498f971682eb5da0f42b9258e67ba6207f12034595a7af7f514f1326957'],
    [currentRouterPrompts.reselection, '5ae80273dbb7ffa32fbbc59216fb47aa0590fff10c97cb9cceb7e9a35442d6b5'],
    [retainedRouterPrompts.direct, '9faed13263187c6a4f4b30688044bd92bf4995edb54c5a7ec7e1158e21414ec0'],
    [retainedRouterPrompts.starter, '2a714646c3920efb762770088e3fbbeed3825556309fe4c94d6c43827879103c'],
    [retainedRouterPrompts.opener, 'b03c74d9510f6651a2b61e4fc66698981ba526865fad77b836ad2198e67efdaa'],
    [retainedRouterPrompts.reselection, '30ccfe940c813c67fa240d1da86c7ee968ca57a5d4ca97b6f9d3d85a223fef48'],
  ...
  if (runtime.conversationPrompt !== reciprocalPrompt || runtime.routerPrompt !== currentRouterPrompts.starter || runtime.router.prompt.sha256 !== hash(currentRouterPrompts.starter)) throw new AppFailure('prompt_hash_mismatch');
...
function routingPrompt(snapshot: Json | undefined, direct: boolean, selected: ReturnType<typeof sessionRuntime>) {
  if (!snapshot || snapshot.router_prompt_version === currentRouterVersion) return direct ? currentRouterPrompts.direct : snapshot?.opener_version === openerVersion ? currentRouterPrompts.opener : currentRouterPrompts.starter;
  if (snapshot.router_prompt_version === retainedRouterVersion) return direct ? retainedRouterPrompts.direct : snapshot?.opener_version === openerVersion ? retainedRouterPrompts.opener : retainedRouterPrompts.starter;
  if (snapshot.router_prompt_version === eightRouterVersion) return direct ? eightRouterPrompts.direct : snapshot?.opener_version === openerVersion ? eightRouterPrompts.opener : eightRouterPrompts.starter;
...
export function routerSnapshot(snapshot?: Json): Json {
  ...
  const compact = !snapshot || [compactRouterVersion, eightRouterVersion, retainedRouterVersion, currentRouterVersion].includes(snapshot.router_prompt_version);
  const promptVersion = !snapshot ? currentRouterVersion : snapshot.router_prompt_version;
  ...
  return { version: compact ? `${promptVersion}_${inputKind}` : direct ? `stomylos_character_router_${directVersion}` : selected.router.version, app_version: appVersion, parameters,
    ...
    response_identity: { allowed_models: selected.router.model.accepted_response_models, provider: selected.router.provider.expected_response_provider }, timeout_seconds: (selected === runtime || selected === conversationV9 || selected === conversationV8) ? 3 : 10,
    ...((selected === runtime || selected === conversationV9 || selected === conversationV8) ? { recovery_version: 'stomylos_router_recovery_v1', recovery_attempt: 0 } : {}) };
}
export function conversationSnapshot(kind?: OpeningKind): Json {
  return { ...structuredClone(runtime.conversation), system_prompt: runtime.conversationPrompt,
    ...
    time_version: timeVersion, component_hashes: conversationComponents(), router_prompt_version: currentRouterVersion,
...
function runtimeForVersion(version: string) {
  ...
  if (version === conversationV9.conversation.version) return conversationV9;
  if (version === conversationV8.conversation.version) return conversationV8;
...
function validateConversationSnapshot(snapshot: Json) {
  ...
  if (modern) {
    if (!(snapshot.memory_version === memoryVersion || ([runtime.conversation.version, conversationV9.conversation.version, conversationV8.conversation.version, conversationV7.conversation.version, conversationV6.conversation.version].includes(snapshot.version) && [sharedMemoryVersion, capacityMemoryVersion, flatMemoryVersion, coldContextVersion].includes(snapshot.memory_version))) || snapshot.time_version !== timeVersion ||
