...
export function validatePatternCommand(name: string, args: unknown) {
  ...
  const keys = name === 'patternCreate' ? ['fingerprint', 'operationId', ...(Object.hasOwn(a, 'selection') ? ['selection'] : [])] : name === 'patternList' ? ['offset', ...(Object.hasOwn(a, 'reportType') ? ['reportType'] : [])] : name === 'patternRetry' ? ['id', 'operationId'] : ['id'];
  if (Object.keys(a).length !== keys.length || keys.some(k => !Object.hasOwn(a, k))) return fail();
  for (const key of keys) {
    if (key === 'reportType') { if (!['grammar','expression'].includes(String(a[key]))) fail(); }
    else if (key === 'selection') { validateSelection(a[key]); }
...
function validateSelection(value: unknown) {
  ...
  if (s.reportType !== undefined && !['grammar','expression'].includes(String(s.reportType))) bad();
  if (Object.keys(s).filter(k=>k!=='reportType').sort().join(',') !== 'excludeCovered,from,timezone,to' || typeof s.excludeCovered !== 'boolean' ||
