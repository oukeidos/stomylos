...
export function renderMemoryBody(doc: MemoryDocument): string {
  return memoryCategories.map(c => c[0].toUpperCase() + c.slice(1) + ':\n' +
    (doc[c].map(i => '- ' + normalizeMemoryText(i.text)).join('\n') || '- None recorded.')).join('\n');
}
export const memoryCharacters = (doc: MemoryDocument) => Array.from(renderMemoryBody(doc)).length;
export function cleanupInput(doc: MemoryDocument): string {
  return memoryCategories.map(c => c[0].toUpperCase() + c.slice(1) + '\n' +
