import step41 from './migrations/041.sql?raw';
import step40 from './migrations/040.sql?raw';
import step39 from './migrations/039.sql?raw';
import step38 from './migrations/038.sql?raw';
import step37 from './migrations/037.sql?raw';
import step36 from './migrations/036.sql?raw';
...
import step43 from './migrations/043.sql?raw';
import step46 from './migrations/046.sql?raw';
import step45 from './migrations/045.sql?raw';
import step44 from './migrations/044.sql?raw';
import { AppFailure } from './errors';

// v0.1.0 ships schema 13. Keep published source schemas and steps immutable.
export const minimumPublicSchema = 13;
export const currentSchema = 46;
const source45 = current.replace(/\n-- Public schema 45 -> 46:[\s\S]*$/, '');
const source44 = current.replace(/\n-- Public schema 44 -> 45:[\s\S]*$/, '');
const source43 = current.replace(/\n-- Public schema 43 -> 44:[\s\S]*$/, '');
const source42 = current.replace(/\n-- Public schema 42 -> 43:[\s\S]*$/, '');
...
export function inspectMigration(db: Database.Database): number {
  ...
  validateSchema(db, version === 13 ? source13 : version < 19 ? source18 : version === 19 ? source19 : version < 22 ? source21 : version < 24 ? source23 : version < 26 ? source25 : version === 26 ? source26 : version === 27 ? source27 : version === 28 ? source28 : version === 29 ? source29 : version === 30 ? source30 : version === 31 ? source31 : version === 32 ? source32 : version === 33 ? source33 : version < 36 ? source35 : version < 38 ? source37 : version === 38 ? source38 : version === 39 ? source39 : version < 43 ? source42 : version === 43 ? source43 : version === 44 ? source44 : version === 45 ? source45 : current);
  integrity(db);
  return version;
}
