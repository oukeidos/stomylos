...
function errorText(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    memory_add_input_limit: 'This conversation exceeds the memory generation input limit. Skip its memory generation to continue.',
    memory_add_format: 'Memory extraction returned an unusable result. Retry or skip this memory source.',
    memory_add_item_capacity: 'An extracted note exceeds 4,000 characters. Retry or skip this memory source.',
    ...
    provider_policy_invalid: 'An internal routing configuration is invalid. No request was sent. Update the app before retrying.',
    provider_source_changed: 'The saved request no longer matches its routing record.',
    database_operation_unsupported: 'An internal application error prevented this operation. Update the app before trying again.',
    ...
    memory_retry_revoked: 'This reply contained a deleted memory and cannot be resent. Start a new chat to continue without it.',
    cold_delete_conflict: 'Older memories changed. Select the note again before deleting it.',
    cold_original_hash: 'An older memory failed its integrity check. The original has been preserved for recovery.',
    cold_snapshot_changed: 'The saved recollection failed its integrity check. Start a new chat or restore a verified backup.',
